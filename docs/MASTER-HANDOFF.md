# 00 — READ THIS FIRST: THIS REPOSITORY IS SHABANEH, NOT PARNIAN

**This document was written for a different site.** It is the handoff for PARNIAN,
a Persian cosmetics template, and it is kept here verbatim because SHABANEH is
built on that codebase: the same Next.js static-export engine, the same content
contract, the same accessibility and RTL rules, the same static-host traps. Every
section below that describes *how the engine works* is still true and still
binding. Every section that describes *what the site looks like* describes the
other site.

SHABANEH is a Persian fine-dining restaurant: dark, cinematic, dinner service
only. No commerce, no price, no cart, no form — the conversion is a phone call.
Two rules were re-decided for it and **supersede the equivalent rules below**.

## The band model replaces the section model

PARNIAN's page is a stack of sections on a light ground: a hero, then alternating
two-column product rows, then a tile grid, inside a shared editorial container.
SHABANEH has none of that. Its page is a stack of **bands**, and a band is:

* `min-height`, never `height` — `calc(var(--band-h) * 1dvh)` with a `1vh`
  fallback, so content taller than the viewport pushes the band open instead of
  being clipped. This is the whole reason the site survives a 390×640 screen.
* full-bleed image, a directional scrim over it, and a small block of text set
  low in the frame. Never a two-column row, never a visible grid, never a card.
* one destination at most. `ProductRow`, which had two links to the same place,
  has been deleted and **its alternating rhythm must not come back in any form**.

Home is five bands, the menu is nine at 70dvh with a course mark between courses,
gallery is full-bleed stacked and explicitly **not** a tile grid. That makes
§04 (homepage structure), §09 (grid system), §10 (product presentation) and the
layout half of §31 historical — read them for the reasoning, not the layout.
`ProductLayout` is gone from `src/types/content.ts` with them, and `tone` has been
repurposed: it no longer names a colour, it selects a scrim weight per course.
See the comment in `src/content/resolveProducts.ts`.

## Contrast is measured over the image, not over the token

§13's accessibility foundation checks text against its background token. On this
site most text sits on a photograph, where the background is whatever pixel
happens to be under the glyph — and a token-based check passes cleanly while a
caption vanishes into a highlight.

So the binding check here is **brightest-pixel over-image contrast**: for every
text node over an image, take its bounding box, draw the underlying image *with
the scrim composited on top* into a canvas, find the **brightest** pixel in that
box — not the average — and measure the text colour against it. Body text needs
4.5:1 against that worst pixel, large text 3:1. `npm run check:bands` runs it
against the served static export and reports the worst ratio per route with the
element that produced it.

Two consequences that look like styling choices but are not:

* **The scrim is a required component**, a directional gradient with a plateau
  under the text, never a flat uniform overlay and never absent. Text on an
  unscrimmed photograph is a defect, not a look.
* **The focus ring carries an opaque halo.** The brass accent measures 2.53:1
  against a blown white highlight, under the 3:1 a focus indicator needs, so the
  ring is drawn dark–brass–dark and stays visible over any photograph.

Every ratio quoted in `src/app/tokens.css` was measured by `npm run check:contrast`,
which is runnable and prints the table. No number in that file is an estimate.
